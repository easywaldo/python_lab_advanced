...
백그라운드에 실행
메인스레드 종료 시 즉시 종료
주로 백그라운드 무한 대기 이벤트 발생 실행하는 부분 담당 -> JVM(가비지 컬렉션), 자동 저장 기능
일반 스레드는 작업 종료 시 까지 실행
"""
...
if __name__ == '__main__':
    ...
    # 함수인지 확인
    x = threading.Thread(target=thread_func, args=('First', range(0,100)), daemon=True)
    y = threading.Thread(target=thread_func, args=('Two',range(0,200)), daemon=True)
